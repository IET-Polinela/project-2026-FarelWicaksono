...
class WorkflowStateTests(APITestCase):
    ...
    def test_WF_02_tidak_bisa_edit_laporan_yang_sudah_reported(self):
        ...
        if warga is None:
            kwargs = {'username': f'warga_wf02_{uuid.uuid4().hex[:8]}'}
            if 'is_admin' in user_fields:
                kwargs['is_admin'] = False
            if 'is_staff' in user_fields:
                kwargs['is_staff'] = False
            if 'is_member' in user_fields:
                kwargs['is_member'] = True
            warga = User.objects.create_user(password='Password123!', **kwargs)

        report = Report.objects.create(
            title='Judul Reported Asli',
            category='Infrastruktur',
            description='Laporan sudah diajukan.',
        ...
        self.assertEqual(report.title, 'Judul Reported Asli')
    ...
    def test_WF_05_laporan_resolved_tidak_bisa_diubah(self):
        ...
        def make_user(prefix, *, admin=False):
            kwargs = {'username': f'{prefix}_{uuid.uuid4().hex[:8]}'}
            if 'is_admin' in user_fields:
                kwargs['is_admin'] = admin
            if 'is_staff' in user_fields:
                kwargs['is_staff'] = admin
            if 'is_member' in user_fields:
                kwargs['is_member'] = not admin
            return User.objects.create_user(password='Password123!', **kwargs)
        ...
        admin = next(
            ...
            None,
        ) or make_user('admin_wf05', admin=True)

        report = Report.objects.create(
            title='Laporan Resolved Terkunci',
            category='Lingkungan',
            description='Konten final tidak boleh berubah.',
            location='Pringsewu',
            status='RESOLVED',
            reporter=warga,
        )
        payload = {
            'title': 'Percobaan Mengubah Resolved',
            'category': report.category,
            'description': 'Percobaan perubahan harus ditolak.',
            'location': report.location,
            'status': 'RESOLVED',
        }
        url = reverse('report-detail', kwargs={'pk': report.pk})

        for actor in (warga, admin):
            self.client.force_authenticate(user=actor)
            response = self.client.put(url, payload, format='json')
            self.assertEqual(response.status_code, 403)

        report.refresh_from_db()
        self.assertEqual(report.title, 'Laporan Resolved Terkunci')
        self.assertEqual(report.description, 'Konten final tidak boleh berubah.')
...
class AdminWorkflowTests(TestCase):
    ...
    def test_WF_03_admin_mengubah_status_reported_ke_verified(self):
        ...
        if admin is None:
            kwargs = {'username': f'admin_wf03_{uuid.uuid4().hex[:8]}'}
            if 'is_admin' in user_fields:
                kwargs['is_admin'] = True
            if 'is_staff' in user_fields:
                kwargs['is_staff'] = True
            if 'is_member' in user_fields:
                kwargs['is_member'] = False
            admin = User.objects.create_user(password='Password123!', **kwargs)

        report = Report.objects.create(
            title='Laporan Menunggu Verifikasi',
            category='Keamanan',
            description='Laporan harus diverifikasi admin.',
        ...
        response = self.client.post(reverse('update_status', kwargs={'pk': report.pk}))
    ...
    def test_WF_04_tidak_ada_transisi_langsung_ke_resolved_dari_reported(self):
        ...
        if admin is None:
            kwargs = {'username': f'admin_wf04_{uuid.uuid4().hex[:8]}'}
            if 'is_admin' in user_fields:
                kwargs['is_admin'] = True
            if 'is_staff' in user_fields:
                kwargs['is_staff'] = True
        ...
        report = Report.objects.create(
            title='Uji Urutan Workflow',
            category='Infrastruktur',
            description='REPORTED harus menuju VERIFIED terlebih dahulu.',
            location='Pesawaran',
            status='REPORTED',
            reporter=None,
        )
