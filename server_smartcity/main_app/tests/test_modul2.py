...
class PrivacyAndDataHidingTests(APITestCase):
    ...
    def test_PRIV_03_tidak_bisa_baca_draf_orang_lain(self):
        ...
        while len(citizens) < 2:
            kwargs = {'username': f'warga_priv03_{uuid.uuid4().hex[:8]}'}
            ...
            citizens.append(User.objects.create_user(password='Password123!', **kwargs))

        warga_a, warga_b = citizens[0], citizens[1]
        draft_b = Report.objects.create(
            title='Draf Privat Warga B',
            category='Infrastruktur',
            description='Draf ini tidak boleh terlihat oleh warga lain.',
            location='Bandar Lampung',
            status='DRAFT',
            reporter=warga_b,
        )

        self.client.force_authenticate(user=warga_a)
        response = self.client.get(reverse('report-detail', kwargs={'pk': draft_b.pk}))
    ...
    def test_PRIV_04_tidak_bisa_modifikasi_draf_orang_lain(self):
        ...
        while len(citizens) < 2:
            kwargs = {'username': f'warga_priv04_{uuid.uuid4().hex[:8]}'}
            if 'is_admin' in user_fields:
                kwargs['is_admin'] = False
            if 'is_staff' in user_fields:
                kwargs['is_staff'] = False
        ...
        warga_a, warga_b = citizens[0], citizens[1]
        draft_b = Report.objects.create(
            title='Judul Draf Asli',
            category='Lingkungan',
            description='Deskripsi asli milik warga B.',
            location='Metro',
            status='DRAFT',
            reporter=warga_b,
        )
        ...
        payload = {
            'title': 'Judul Disusupi Warga A',
            'category': draft_b.category,
            'description': 'Konten ini tidak boleh tersimpan.',
            'location': draft_b.location,
            'status': 'DRAFT',
        }

        self.client.force_authenticate(user=warga_a)
        response = self.client.put(
            reverse('report-detail', kwargs={'pk': draft_b.pk}),
            payload,
            format='json',
        )
        self.assertEqual(response.status_code, 404)
        ...
        self.assertEqual(draft_b.title, 'Judul Draf Asli')
        self.assertEqual(draft_b.description, 'Deskripsi asli milik warga B.')